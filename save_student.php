...
if(empty($id)){
	...
	if($insert_user){
		$id = $conn->insert_id;
		$insert_students =$conn->query("INSERT INTO students set user_id = '".$id."', grade_section='".$level_section."' ");
		if($insert_students){
	...
	if($update_user){
		$update_students =$conn->query("UPDATE students set grade_section='".$level_section."' where id = '".$id."' ");
		if($update_students){
			echo json_encode(array('status'=>1));
		}
	}
