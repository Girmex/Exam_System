...
<body style="display:flex">
	...
	<div class="container-fluid admin">
		<div class="col-md-12 alert alert-primary">Exam Records</div>
		<br>
		<div class="col-md-4 offset-md-4 mb-4">
			<select class="form-control select2" onchange="location.replace('history.php?exam_id='+this.value)">
				<option value="all" <?php echo isset($_GET['exam_id']) && $_GET['exam_id'] == 'all' ? 'selected' : '' ?>>All</option>
				<?php 
				$where =''; 
				if($_SESSION['login_user_type'] == 2){
					$where = ' where user_id = '.$_SESSION['login_id'].' '; 
				}
				$quiz = $conn->query("SELECT * FROM exam_list ".$where." order by title asc");
				while($row = $quiz->fetch_assoc()){
				?>
				<option value="<?php echo $row['id'] ?>" <?php echo isset($_GET['exam_id']) && $_GET['exam_id'] == $row['id']  ? 'selected' : '' ?>><?php echo $row['title'] ?></option>
			<?php } ?>
			</select>
		</div>
		<div class="card">
			<div class="card-body">
				<table class="table table-bordered" id='table'>
					...
					$where = '';
					if($_SESSION['login_user_type'] == 2){
						$where = ' where e.user_id = '.$_SESSION['login_id'].' ';
					}
					if(isset($_GET['exam_id']) && $_GET['exam_id'] != 'all'){
						if(empty($where)){
						$where = ' where e.id = '.$_GET['exam_id'].' ';

						}else{
						$where = ' and e.id = '.$_GET['exam_id'].' ';

						}
					}
					$qry = $conn->query("SELECT h.*,u.name as student,e.title from history h inner join users u on h.user_id = u.id inner join exam_list e on h.exam_id = e.id ".$where." order by u.name asc ");
					$i = 1;
					if($qry->num_rows > 0){
